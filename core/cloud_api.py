...
class CloudAPIClient:
    ...
    def list_spaces(self) -> list:
        ...
        response = self._request("GET", "/api/v1/spaces")
        data = response.json()
    ...
    def create_space(self, name: str, type_: str) -> dict:
        ...
        response = self._request("POST", "/api/v1/spaces", json=payload)
        return response.json()
    ...
    def delete_space(self, space_id: str) -> None:
        ...
        self._request("DELETE", f"/api/v1/spaces/{space_id}")

    @requires_plugin_permission("network")
    def list_space_members(self, space_id: str) -> list:
        """返回 space 成员列表。"""
        response = self._request("GET", f"/api/v1/spaces/{space_id}/members")
        data = response.json()
        if isinstance(data, dict):
    ...
    def remove_space_member(self, space_id: str, user_id: str) -> None:
        ...
        self._request("DELETE", f"/api/v1/spaces/{space_id}/members/{user_id}")

    @requires_plugin_permission("network")
    def leave_space(self, space_id: str) -> None:
        """当前用户主动退出 space。owner 需先转让或删除。"""
        self._request("POST", f"/api/v1/spaces/{space_id}/leave")
    ...
    def list_share_links(self) -> list:
        """列出当前用户创建的分享链接。"""
        response = self._request("GET", "/api/v1/share-links")
        data = response.json()
        if isinstance(data, dict):
    ...
    def create_share_link(
        ...
        response = self._request("POST", "/api/v1/share-links", json=payload)
        return response.json()

    @requires_plugin_permission("network")
    def revoke_share_link(self, share_id: str) -> None:
        """吊销分享链接。"""
        self._request("DELETE", f"/api/v1/share-links/{share_id}")
    ...
    def view_share_link(self, token: str) -> dict:
        ...
        """
        try:
            resp = self._client.get(f"/api/v1/share-links/view/{token}")
        except httpx.TimeoutException:
            raise CloudAPIError("请求超时，请检查网络连接")
        except httpx.ConnectError:
            raise CloudAPIError("无法连接到云端服务器，请检查网络")
