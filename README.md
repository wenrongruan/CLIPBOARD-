...
- 边缘停靠、系统托盘、全局热键。
- 本地 SQLite 存储。
- 截图：区域 / 全屏 / 窗口三种取景，热键或托盘菜单触发，结果直接进历史并复制到剪贴板。
...
- `core/`：数据库、仓库、模型、同步、云端 API、插件系统；截图取景在 `core/screenshot_service.py`（macOS 调系统 `screencapture`，其他平台走 Qt 抓屏 + `ui/screenshot_overlay.py` 的框选遮罩）。
