...
core/
  ...
  plugins/
    plugin_manager.py   保持现有外部入口，内部精简
    extension_points.py 新增：ExtensionPointRegistry
...
### 4.4 `core/plugins/extension_points.py`
...
1. **大推一次到位，无中间提交可二分**
   - 缓解：本地分支 `refactor/foundation` + 每柱完成打本地 tag（pillar-1-done…pillar-7-done）
